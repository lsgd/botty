...
export class TranscriptionPlugin {
    ...
    async onCommand(command, args, message) {
        ...
        if (
            command === this.commands[0].command ||
            command === this.commands[1].command
        ) {
    ...
    async handleTranscriptionSettings(args, message) {
        ...
        if (subCommand === "global") {
            if (args.length < 2) {
                await responseHelper.reply(
                    message,
                    i18n.t("settingsUsageError"),
                );
                return;
            }
    ...
    async transcribeMessage(message) {
        try {
            ...
            await fs.writeFile(tempFilePath, media.data, {
                encoding: "base64",
            });
